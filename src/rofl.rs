...
pub struct PlayBack {
    ...
    pub num_chunks: u32,
    pub end_startup_chunk: u32,
    pub game_start_chunk: u32,
    pub keyframe_time_interval: u32,
}

pub struct Rofl {
    pub game_metadata: String,
    pub playback_metadata: PlayBack,
...
impl Packet {
    pub fn parse_packets(source: &[u8], start_time: f32) -> io::Result<Vec<Packet>> {
        let mut results = Vec::new();
        let mut time = start_time;
        let mut id = 0u16;
        let mut param = 0u32;
        let mut reader = Cursor::new(source);
        while let Ok(marker) = reader.read_u8() {
            let channel = marker & 0xF;
            let flags = marker >> 4;
            if flags & 0x08 == 0 {
                time = reader.read_f32::<LE>()?;
            } else {
                time += reader.read_u8()? as f32 / 1000.0f32;
            }
            let length = if flags & 0x01 == 0 {
                reader.read_u32::<LE>()? as usize
            } else {
                reader.read_u8()? as usize
            };
            if flags & 0x04 == 0 {
                id = reader.read_u16::<LE>()?;
            }
            if flags & 0x02 == 0 {
                param = reader.read_u32::<LE>()?;
            } else {
                param = param.wrapping_add(reader.read_u8()? as u32);
            }
            let mut data = Vec::<u8>::with_capacity(length + 6);
            data.extend_from_slice(&u16::to_le_bytes(id));
            data.extend_from_slice(&u32::to_le_bytes(param));
            reader.clone().take(length as u64).read_to_end(&mut data)?;
            reader.seek(io::SeekFrom::Current(length as i64))?;
            results.push(Packet {
                time,
                channel,
                id,
...
impl Chunk {
    pub fn parse_packets(&self, start_time: f32) -> io::Result<Vec<Packet>> {
        Packet::parse_packets(&self.data, start_time)
    }
}

impl Rofl {
    pub fn read<R: ReadBytesExt + Seek>(reader: &mut R) -> io::Result<Rofl> {
        let raw = RawRofl::read(reader)?;
        let chunks = {
            let mut chunks = Vec::new();
            let blowfish = raw.make_blowfish();
            let data_offset = raw.data_offset;
            for raw_chunk in raw.chunks {
                let id = raw_chunk.id;
                let kind = match raw_chunk.kind {
                    1 => ChunkKind::Common,
                    2 => ChunkKind::Snapshot,
                    3 => ChunkKind::EndStartup,
                    4 => ChunkKind::GameStart,
                    5 => ChunkKind::Startup,
                    _ => ChunkKind::None,
                };
                let next_id = raw_chunk.next_id;
                let size = raw_chunk.size as usize;
                let offset = raw_chunk.offset as u64;
                let mut raw_data = vec![0u8; size];
                reader.seek(SeekFrom::Start(data_offset + offset))?;
                reader.read_exact(&mut raw_data)?;
                blowfish.decrypt(&mut raw_data);
                let mut data = Vec::new();
                libflate::gzip::Decoder::new(io::Cursor::new(&raw_data))
                    .unwrap()
        ...
        let game_metadata = raw.game_metadata;
        let playback_metadata = raw.playback_metadata;
        Ok(Rofl {
            game_metadata,
            playback_metadata,
            chunks,
    ...
    pub fn from_file<F: AsRef<Path>>(name: F) -> io::Result<Rofl> {
        let mut file = File::open(name)?;
        let rofl = Rofl::read(&mut file)?;
        Ok(rofl)
    }

    pub fn dump_packets(&self, drop_snapshots: bool) -> io::Result<Vec<Packet>> {
        let mut result = Vec::new();
        for chunk in &self.chunks {
            if drop_snapshots && chunk.kind == ChunkKind::Snapshot {
                continue;
            }
            for packet in chunk.parse_packets(0.0)? {
                result.push(packet);
            }
        }
        Ok(result)
...
impl RawRofl {
    pub fn read<R: ReadBytesExt + Seek>(reader: &mut R) -> io::Result<RawRofl> {
        let _magic = reader.read_u32::<LE>()?;
        let _version = reader.read_u16::<LE>()?;
        let mut _signature = [0u8; 256];
        reader.read_exact(&mut _signature)?;
        let _header_size = reader.read_u16::<LE>()?;
        let _file_size = reader.read_u32::<LE>()?;
        let game_metadata_offset = reader.read_u32::<LE>()? as u64;
        let game_metadata_size = reader.read_u32::<LE>()? as usize;
        let playback_metadata_offset = reader.read_u32::<LE>()? as u64;
        let _playback_metadata_size = reader.read_u32::<LE>()? as usize;
        let chunks_offset = reader.read_u32::<LE>()? as u64;
        let game_metadata = {
            reader.seek(SeekFrom::Start(game_metadata_offset))?;
            let mut buffer = vec![0u8; game_metadata_size];
            reader.read_exact(&mut buffer)?;
            String::from_utf8_lossy(&buffer).to_string()
        };
        let playback_metadata = {
            reader.seek(SeekFrom::Start(playback_metadata_offset))?;
            let game_id = reader.read_u64::<LE>()?;
            let match_length = reader.read_u32::<LE>()?;
            let num_key_frames = reader.read_u32::<LE>()?;
            let num_chunks = reader.read_u32::<LE>()?;
            let end_startup_chunk = reader.read_u32::<LE>()?;
            let game_start_chunk = reader.read_u32::<LE>()?;
            let keyframe_time_interval = reader.read_u32::<LE>()?;
            PlayBack {
                game_id,
                match_length,
                num_key_frames,
        ...
        let key = {
            ...
            let mut buffer = vec![0u8; key_length];
            reader.read_exact(&mut buffer)?;
            buffer.into_boxed_slice()
        };
        let chunks = {
            reader.seek(SeekFrom::Start(chunks_offset))?;
            let mut chunks = Vec::new();
            for _ in 0..(playback_metadata.num_chunks + playback_metadata.num_key_frames) {
                let id = reader.read_u32::<LE>()?;
                let kind = reader.read_u8()?;
                let size = reader.read_u32::<LE>()?;
                let next_id = reader.read_u32::<LE>()?;
        ...
        // NOTE: this should ideally be fetched with .tell()
        let data_offset = chunks_offset + (17 * chunks.len() as u64);
        Ok(RawRofl {
            game_metadata,
            playback_metadata,
            key,
    ...
    pub fn make_blowfish(&self) -> BlowFish {
        if self.key.len() > 16 {
            let small_key = format!("{}", self.playback_metadata.game_id);
            let mut enc_key = base64::decode(&self.key).unwrap();
            let blowfish = BlowFish::new(small_key.as_bytes());
            blowfish.decrypt(&mut enc_key);
            enc_key.resize(16, 0u8);
            BlowFish::new(&enc_key)
        } else {
            BlowFish::new(&self.key)
        }
    }
}
...
impl BlowFish {
    pub fn new(key: &[u8]) -> BlowFish {
        let mut state = BlowFish {
            pikey: BlowFish::DEFAULT_PI_KEY,
            sboxes: BlowFish::DEFAULT_S_BOXES,
        };
        let key_size = usize::min(key.len(), 56);
        for i in 0usize..18usize {
            let mut bytes = [0u8; 4];
            for b in 0usize..4usize {
                bytes[b] = key[((i * 4) + b) % key_size];
            }
            state.pikey[i] ^= u32::from_be_bytes(bytes);
        }
        let mut block = (0, 0);
        for i in 0usize..9usize {
            block = state.encrypt_block(block);
            state.pikey[i * 2] = block.0;
            state.pikey[i * 2 + 1] = block.1;
        }
        for i in 0usize..4usize {
            for j in 0usize..128usize {
                block = state.encrypt_block(block);
                state.sboxes[i][j * 2] = block.0;
                state.sboxes[i][j * 2 + 1] = block.1;
            }
        }
        state
    }
    ...
    pub fn decrypt(&self, data: &mut [u8]) {
        data.chunks_exact_mut(8).for_each(|chunk| {
            let mut lo = [0u8; 4];
            lo.copy_from_slice(&chunk[..4]);
            let mut hi = [0u8; 4];
            hi.copy_from_slice(&chunk[4..]);
            let block_in = (u32::from_be_bytes(lo), u32::from_be_bytes(hi));
            let block_out = self.decrypt_block(block_in);
            chunk[..4].copy_from_slice(&block_out.0.to_be_bytes());
            chunk[4..].copy_from_slice(&block_out.1.to_be_bytes());
        });
    }

    fn encrypt_block(&self, block: (u32, u32)) -> (u32, u32) {
        let (mut lo, mut hi) = block;
        lo ^= self.pikey[0];
        hi ^= self.feistel(lo) ^ self.pikey[1];
        lo ^= self.feistel(hi) ^ self.pikey[2];
        hi ^= self.feistel(lo) ^ self.pikey[3];
        lo ^= self.feistel(hi) ^ self.pikey[4];
        hi ^= self.feistel(lo) ^ self.pikey[5];
        lo ^= self.feistel(hi) ^ self.pikey[6];
        hi ^= self.feistel(lo) ^ self.pikey[7];
        lo ^= self.feistel(hi) ^ self.pikey[8];
        hi ^= self.feistel(lo) ^ self.pikey[9];
        lo ^= self.feistel(hi) ^ self.pikey[10];
        hi ^= self.feistel(lo) ^ self.pikey[11];
        lo ^= self.feistel(hi) ^ self.pikey[12];
        hi ^= self.feistel(lo) ^ self.pikey[13];
        lo ^= self.feistel(hi) ^ self.pikey[14];
        hi ^= self.feistel(lo) ^ self.pikey[15];
        lo ^= self.feistel(hi) ^ self.pikey[16];
        hi ^= self.pikey[17];
        (hi, lo)
    }

    fn decrypt_block(&self, block: (u32, u32)) -> (u32, u32) {
        let (mut lo, mut hi) = block;
        lo ^= self.pikey[17];
        hi ^= self.feistel(lo) ^ self.pikey[16];
        lo ^= self.feistel(hi) ^ self.pikey[15];
        hi ^= self.feistel(lo) ^ self.pikey[14];
        lo ^= self.feistel(hi) ^ self.pikey[13];
        hi ^= self.feistel(lo) ^ self.pikey[12];
        lo ^= self.feistel(hi) ^ self.pikey[11];
        hi ^= self.feistel(lo) ^ self.pikey[10];
        lo ^= self.feistel(hi) ^ self.pikey[9];
        hi ^= self.feistel(lo) ^ self.pikey[8];
        lo ^= self.feistel(hi) ^ self.pikey[7];
        hi ^= self.feistel(lo) ^ self.pikey[6];
        lo ^= self.feistel(hi) ^ self.pikey[5];
        hi ^= self.feistel(lo) ^ self.pikey[4];
        lo ^= self.feistel(hi) ^ self.pikey[3];
        hi ^= self.feistel(lo) ^ self.pikey[2];
        lo ^= self.feistel(hi) ^ self.pikey[1];
        hi ^= self.pikey[0];
        (hi, lo)
    }

    fn feistel(&self, val: u32) -> u32 {
        let i = u32::to_be_bytes(val);
        let s0 = self.sboxes[0][i[0] as usize];
        let s1 = self.sboxes[1][i[1] as usize];
        let s2 = self.sboxes[2][i[2] as usize];
        let s3 = self.sboxes[3][i[3] as usize];
        (s0.wrapping_add(s1) ^ s2).wrapping_add(s3)
    }

    const DEFAULT_PI_KEY: [u32; 18] = [
    ...
    const DEFAULT_S_BOXES: [[u32; 256]; 4] = [
        [
